...
Имеется таблица PRPD-признаков, извлеченных из сигналов частичных
разрядов. Каждое наблюдение относится к одному из диагностических классов:
`no_pd`, `corona`, `surface`, `internal`.

Требуется:

1. обучить классификатор типа дефекта;
2. сравнить несколько моделей;
3. объяснить диагностический смысл ошибок;
4. показать, почему масштабирование необходимо для SVM.

<!--
Задача формулируется как предварительная классификация. Преподаватель
должен подчеркнуть, что это не заменяет регламентированные испытания
изоляции и экспертную диагностику.
-->
...
\qquad y_i \in \{\text{no\_pd}, \text{corona}, \text{surface}, \text{internal}\}.
$$
...
и оценить качество по accuracy и macro-F1. Macro-F1 важна, потому что
она усредняет качество по классам.

<!--
Формальная запись связывает занятие 5 с занятиями 2 и 3. Важно
объяснить, что целевая переменная здесь категориальная, а не непрерывная.
-->
...
2. Выбрать безопасные PRPD-признаки.
3. Выполнить `StandardScaler`.
4. Обучить Logistic Regression, Decision Tree, Random Forest и SVM.
5. Рассчитать accuracy, macro-precision, macro-recall и macro-F1.
6. Построить матрицу ошибок.
7. Сравнить SVM с масштабированием и без масштабирования.
8. Объяснить утечку через `risk_score`.

<!--
Список шагов задает структуру отчета. Если студент не успевает выполнить
эксперимент с параметром C, приоритет сохраняется за масштабированием,
матрицей ошибок и объяснением risk_score.
-->
...
SVM (Support Vector Machine), метод опорных векторов, ищет разделяющую
поверхность с максимальным зазором (margin):

$$
\mathbf{w}^{T}\mathbf{x} + b = 0.
$$

Для нелинейной границы используется ядро RBF (Radial Basis Function),
радиальная базисная функция.

<!--
Пояснить, что SVM зависит от расстояний между объектами. Поэтому признаки
в градусах, пикокулонах и герцах сначала приводятся к сопоставимому
масштабу.
-->
...
Если обучить SVM на признаках с разными физическими единицами без
`StandardScaler`, расстояния и зазор будут определяться признаками с
наибольшим численным диапазоном.

Сравнение в блокноте:

1. `svm_rbf_with_scaler` - корректный pipeline;
2. `svm_rbf_without_scaler` - методический антипример.

<!--
Этот слайд закрывает отдельный шаг PLAN.md. Важно сказать, что даже если
на учебных данных качество без масштабирования случайно выглядит высоким,
методически такой pipeline остается неправильным.
-->
