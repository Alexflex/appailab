...
Имеется таблица сенсорных признаков режимов оборудования. Целевая
переменная при обучении отсутствует.

Требуется:

1. сгруппировать наблюдения по сходству признаков;
2. выбрать рациональное число кластеров;
3. сравнить k-means, DBSCAN и Gaussian Mixture;
4. дать инженерное описание найденных групп;
5. использовать diagnostics-CSV только после обучения.

<!--
Этот слайд предотвращает типовую ошибку: студент пытается "предсказать"
true_mode_label. Нужно подчеркнуть, что true_mode_label скрыт до этапа
интерпретации.
-->
...
Дано множество наблюдений без целевой переменной:
...
$$
c_i = g(\mathbf{x}_i), \qquad c_i \in \{1,\ldots,k\},
$$

где $c_i$ - номер кластера. Качество оценивается по силуэтному
коэффициенту и инженерной интерпретации профилей кластеров.

<!--
Формальная постановка показывает отличие от классификации: нет y_i.
Diagnostics-CSV может использоваться только для последующей проверки
смысла найденных c_i.
-->
...
3. Выполнить масштабирование.
4. Построить корреляции, PCA и PCA biplot.
5. Подобрать число кластеров по inertia и silhouette.
6. Сравнить k-means, DBSCAN и Gaussian Mixture.
7. Описать 2-3 кластера по профилям признаков.
8. Объяснить антипример скрытой классификации.

<!--
Список шагов нужен как дорожная карта занятия. Студент должен видеть, что
главный результат - интерпретация кластеров, а не только запуск
алгоритма.
-->
...
Метод k-средних минимизирует сумму квадратов расстояний до центров:
...
Так как функция использует расстояния, признаки должны быть
масштабированы до сопоставимых диапазонов.

<!--
Этот слайд связывает математическую запись с практическим действием
StandardScaler. Без масштабирования ток, скорость или давление могут
искусственно доминировать в расстоянии.
-->

---

## DBSCAN и Gaussian Mixture

DBSCAN выделяет плотные области и помечает разреженные точки как шум
меткой `-1`.

Gaussian Mixture, гауссова смесь, описывает данные как смесь нормальных
распределений и назначает объекту наиболее вероятный компонент.

Эти методы сравниваются с k-means, чтобы показать разные предположения о
форме кластеров.

<!--
Сделать акцент на предположениях моделей: k-means предпочитает компактные
группы вокруг центров, DBSCAN ищет плотность, GMM задает вероятностные
компоненты.
-->
